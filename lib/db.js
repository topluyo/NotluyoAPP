...
/**
 * Lazy cleanup: delete all expired shared temp bords.
 * Called automatically before temp bord reads and storage checks.
 * Non-blocking — errors are silently ignored so the main request isn't affected.
 */
export async function cleanupExpiredTemps() {
  try {
    const sql = getDb();
    await sql`DELETE FROM shared_temp_bords WHERE expires_at < NOW()`;
  } catch {
 ...
 * Also triggers lazy cleanup of expired temp bords.
 */
