...
export default async function BordPage({ params }) {
  ...
  const info = {
    ...
    mode,
    isOwner,
    user: { nick: bord.user_nick, name: bord.user_name },
    currentUser: user ? { id: user.id, nick: user.nick, name: user.name } : null,
    apiBase: "/api",
      ...
      <body style={{ margin: 0, padding: 0, background: "#000", overflow: "hidden" }}>
        ...
        <script
          dangerouslySetInnerHTML={{
            __html: `window.info=${JSON.stringify(info)};window.bord=${JSON.stringify(bord.data)};`,
          }}
        />
