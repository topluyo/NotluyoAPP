...
///@test
// bord.load("http://localhost:3000/bords/test/maaliyet.json")
...
//@edit
  {
    ...
    let that=PARENT=presentation.container=Asena(`<div class="bord--container bord--presentation--coordinates-container no-action" style='display:none'></div>`);
    {
    let PARENT = that;{
      ...
      let PARENT = that;{
        ...
        let PARENT = that;{
          ...
          let that=presentation.elements.coordinatesAddButton=Asena(`<icon name="plus" class="bord--button--icon"></icon>`);
          PARENT.append(that);
        {

          let that=presentation.elements.coordinatesSortButton=Asena(`<icon name="sort" class="bord--button--icon"></icon>`);
          PARENT.append(that);
        presentation.elements.coordinatesSortButton.onclick = function () { 
          presentation.elements.coordinatesList.classList.toggle("sort") 
          presentation.elements.coordinatesList.sortableGrid.enabled = presentation.elements.coordinatesList.classList.contains("sort")    
        }
        {

          let that=_temp=Asena(`<span>Coordinates</span>`);
          PARENT.append(that);
        }
        }
        }
      ...
      }
    {
      ...
      let that=_temp=Asena(`<div class="bord--container--body" style='overflow-x:hidden;'></div>`);
      PARENT.append(that);
      {
      let PARENT = that;{

        let that=presentation.elements.coordinatesList=Asena(`<div class="bord--presentation--coordinates-container-list no-action"></div>`);
        PARENT.append(that);
      SortableGrid(presentation.elements.coordinatesList, {useCloser:false,items:'.bord--presentation--coordinates-container--item'})
      presentation.elements.coordinatesList.sortableGrid.enabled = false
      presentation.elements.coordinatesList.sortableGrid.onSorted = function (items) {
        vision.coordinates = items.map(e=>e.data)
  ...
  presentation.elements.coordinatesAddButton.onclick = function () {
    vision.add.coordinate()
    presentation.refresh()
  }
  vision.bord.parent.append(presentation.container)
//@endedit
...
//@edit
    presentation.elements.coordinatesList.innerHTML = ""

    for(let i = 0 ; i < datas.length ; i++){
      let data = datas[i]
      let editItem,sortItem,targetItem,textItem,deleteItem,goItem
      {

        let that=PARENT=editItem = Asena(`<div class="bord--presentation--coordinates-container--item" style='display: flex;flex-direction: row;align-items: center;gap:4px;padding:2px 2px;'></div>`);
        {
        let PARENT = that;{

          let that=sortItem = Asena(`<icon name='line2' class="bord--button--icon--hover"></icon>`);
          PARENT.append(that);
        {

          let that=targetItem = Asena(`<icon name='target' class="bord--button--icon--hover"></icon>`);
          PARENT.append(that);
        {

          let that=goItem = Asena(`<icon name='play' class="bord--button--icon--hover"></icon>`);
          PARENT.append(that);
        {

          let that=textItem = Asena(`<div style='flex:1;font-weight:400;padding:4px;font-size:14px;' contenteditable>${data.name}</div>`);
          PARENT.append(that);
        {

          let that=deleteItem = Asena(`<icon name='trash' class="bord--button--icon--hover"></icon>`);
          PARENT.append(that);
        }
        }
        }
      ...
      textItem.onblur = function(){
        data.name = textItem.innerText
        presentation.refresh()
      }
      deleteItem.onclick=()=>{
        vision.coordinates.splice(i,1)
        presentation.refresh()
      }
      targetItem.onclick=()=>{
        data.x = vision.camera.x
        data.y = vision.camera.y
        data.s = vision.camera.s
      }
      goItem.onclick=()=>{
        presentation.go(i)
      }
      presentation.elements.coordinatesList.append(editItem)
...
//@test
      ...
      }
    {
      ...
      let that=_example=Asena(`<div class="bord--button--round" style="position:absolute;right:200px;top:0px;"></div>`);
      PARENT.append(that);
      {
      let PARENT = that;{

        let that=_example=Asena(`<icon name="play"></icon>`);
        PARENT.append(that);
//@endtest
...
//--- Line Color ---
      ...
      }
    {
      ...
      let PARENT = that;{
        ...
        let that=vision.elements.buttonColor=Asena(`<div id="bord--button--color" class="bord--button--round"></div>`);
        PARENT.append(that);
        {
        let PARENT = that;{

          let that=_temp=Asena(`<icon name="pencil"></icon>`);
          PARENT.append(that);
        }
        }
      {
        ...
        PARENT.append(that);
        var list=["D","Q","A","Z","X","C"]
        var commandList=["Siyah,Beyaz","Kırmızı","Sarı","Yeşil","Mavi","Mor"]
        for(let i=1;i<7;i++){
          {
          let PARENT = that;{

            let that=_temp=Asena(`<div class="bord--button--round" style="background:var(--color-${i})" key="Key${list[i-1]}" command="${commandList[i-1]}"></div>`);
            PARENT.append(that);
          _temp.onclick=_temp.action=()=>{ vision.color.set(i) }
          }
          }
        }
...
//-- Line Stroke ---
      ...
      }
    {
      ...
      let that=vision.elements.buttonRightStroke=Asena(`<div class="bord--buttons--right" style="position:absolute;left:0px;top:100px;font-size:24px"></div>`);
      PARENT.append(that);
      {
      let PARENT = that;{

        let that=vision.elements.buttonStroke=Asena(`<div class="bord--button--round"></div>`);
        PARENT.append(that);
        {
        let PARENT = that;{

          let that=_temp=Asena(`<icon name="stroke_1"></icon>`);
          PARENT.append(that);
        }
        }
      {
        ...
        let that=_temp=Asena(`<div class="bord--buttons--container"></div>`);
        PARENT.append(that);
        var list = {1:4,2:7,3:11,4:16,5:22,6:30}
        var commandList=["İnce","İnce","Orta","Orta","Kalın","Kalın"]
        for(let index in list){
          let stroke = list[index]
          {
          let PARENT = that;{

            let that=button=Asena(`<div class="bord--button--round" style="" key="Digit${index}" command="${commandList[index-1]}"></div>`);
            PARENT.append(that);
            {
            let PARENT = that;{

              let that=_temp=Asena(`<icon name="stroke_${index}"></icon>`);
              PARENT.append(that);
            }
            }
          button.onclick=button.action=(e)=>{ vision.stroke.set(stroke); }
          }
          }
        }
      ...
      }
    {
      ...
      let that=_temp=Asena(`<div class="bord--button--round" style="position:absolute;left:0px;top:200px;" key="KeyE" command="Sil"></div>`);
      PARENT.append(that);
      that.onclick=()=>{ vision.removeSelectedElements() }
      {
      let PARENT = that;{

        let that=_temp=Asena(`<icon name="eraser"></icon>`);
        PARENT.append(that);
      ...
      }
    {
      ...
      let that=vision.elements.enableButton=Asena(`<div class="bord--button--round" style="position:absolute;left:0px;top:250px;background:#2C2;color:white;"></div>`);
      PARENT.append(that);
      that.onclick=()=>{ vision.disabled=!vision.disabled; }
      {
      let PARENT = that;{

        let that=_temp=Asena(`<icon name="change"></icon>`);
        PARENT.append(that);
...
//--- Theme ---//
      ...
      }
    {
      ...
      let that=vision.elements.buttonRightTheme=Asena(`<div class="bord--buttons--right" style="position:absolute;left:0px;top:150px;"></div>`);
      PARENT.append(that);
      {
      let PARENT = that;{
      ...
      }
    {
      ...
      let that=_temp=Asena(`<div class="bord--button--round" style="position:absolute;right:200px;top:0px"></div>`);
      PARENT.append(that);
      that.onclick=()=>{
        return Bord.Vision.Add();

// TODO : Module Based
        let type = prompt("Type : camera, video, pdf, paper, image, iframe,code/python","image")
        if(type == "camera"){
          vision.add.newMedia("camera",{},true)
//vision.add.camera()
        }else if(type=="coordinate"){
          vision.add.coordinate()
        }else if(type=="video"){
          let url = prompt("Dosya url","https://app.nightbord.com/bords/hasandelibas/files/reyhan/merhaba.mp4")
          vision.add.newMedia("video",{url:url},true)
        }else if(type=="pdf"){
          Bord.Pdf.Add(vision.bord)
        }else if(type=="paper"){
          Bord.Pdf.Add(vision.bord,{gray:true,invertable:true})
        }else if(type=="image"){
          WebEditable.FileManager.Select("image").then((file)=>{
// Only one
            vision.add.newMedia("image",{url:'files/'+file[0]},true)
          ...
          let url = prompt("Hedef url","https://www.youtube.com/embed/v6NAZOXo5iI")
          vision.add.newMedia("iframe",{url:url},true)
        }else if(type=="code/python"){
          let url = "/source/Bord/Vision/Medias/CodeEditor/index.html?language=python"
          vision.add.newMedia("iframe",{url:url},true)
        }
      }
      ...
      let PARENT = that;{
        ...
        let that=_temp=Asena(`<icon name="plus"></icon>`);
        PARENT.append(that);
...
//--- Save ---//
      ...
      }
    {
      ...
      let that=vision.elements.saveButton=Asena(`<div class="bord--button--round" key="Ctrl-KeyS" style="position:absolute;top:0px;right:100px;"></div>`);
      PARENT.append(that);
      that.onclick=()=>{
        let bordname = location.pathname.substring(location.pathname.indexOf('@')+1)
        vision.save().then(data=>{
          fetch("/api/save",{
            method:"POST",
            headers:{
              "Content-Type":"application/json"
            ...
            vision.elements.saveButton.loadingIcon.style.display="none"
            vision.elements.saveButton.successIcon.style.display=null
            setTimeout(()=>{
              vision.elements.saveButton.saveIcon.style.display=null
              vision.elements.saveButton.successIcon.style.display="none"
            },2000);
          })
        })


        vision.elements.saveButton.loadingIcon.style.display=null
        vision.elements.saveButton.saveIcon.style.display="none"
      } 
      {
      let PARENT = that;{

        let that=vision.elements.saveButton.saveIcon=Asena(`<icon name="save" class="animation-fade-in"></icon>`);
        PARENT.append(that);
      {

        let that=vision.elements.saveButton.loadingIcon=Asena(`<icon name="loading" class="animation-spin" style="display:none;color:green;font-size:1.25em;"></icon>`);
        PARENT.append(that);
      {

        let that=vision.elements.saveButton.successIcon=Asena(`<icon name="check" class="animation-zoom-in" style="display:none;color:green;font-size:1.25em;"></icon>`);
        PARENT.append(that);
...
//-- Plugins --//
      ...
      }
    {
      ...
      let that=vision.elements.pluginsButton=Asena(`<div class="bord--buttons--down" style="position:absolute;right:150px;top:0px;"></div>`);
      PARENT.append(that);
      {
      let PARENT = that;{

        let that=_temp=Asena(`<div class="bord--button--round"></div>`);
        PARENT.append(that);
        {
        let PARENT = that;{

          let that=_temp=Asena(`<icon name="plugin"></icon>`);
          PARENT.append(that);
        }
        }
      {
        ...
        let PARENT = that;{
          ...
          let that=_temp=Asena(`<div class="bord--button--round" style=""></div>`);
          PARENT.append(that);
          that.onclick=()=>{ vision.bord.presentation.container.toggle() }
          {
          let PARENT = that;{

            let that=_temp=Asena(`<icon name="target"></icon>`);
            PARENT.append(that);
          }
          }
        {
          ...
          let that=_temp=Asena(`<div class="bord--button--round" style=""></div>`);
          PARENT.append(that);
          that.onclick=()=>{ bord.assistant.designer.container.toggle() }
          {
          let PARENT = that;{

            let that=_temp=Asena(`<icon name="lamp"></icon>`);
            PARENT.append(that);
...
//@test
      ...
      }
    {
      ...
      let that=vision.elements.debug=Asena(`<pre style="position:absolute;bottom:20px;left:20px;right:20px;color:aqua;font-size:10px;display:none"></pre>`);
      PARENT.append(that);
//@endtest
